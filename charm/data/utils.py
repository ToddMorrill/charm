...
def load_release_metadata(raw_data_dir):
    ...
    for release in release_filepaths:
        if not os.path.exists(release_filepaths[release]):
            logging.warning(f'Release {release_filepaths[release]} does not exist and is being skipped.')
            continue
...
def load_ldc_annotations(raw_data_dir):
    ...
    # anno folders
    annos = {'Annotation-1': 'LDC2022E18_CCU_TA1_Mandarin_Chinese_Development_Annotation_V3.0', 
            'Mini-Eval-Annotations': 'LDC2023E01_CCU_TA1_Mandarin_Chinese_Mini_Evaluation_Annotation_Unsequestered'}
    anno_filepaths = {}
    for anno in annos:
    ...
    for anno in anno_filepaths:
        if not os.path.exists(anno_filepaths[anno]):
            logging.warning(f'Release {anno_filepaths[anno]} does not exist and is being skipped.')
            continue
        anno_dfs_, segment_df, versions_df = load_ldc_annotation(anno_filepaths[anno])
        anno_dfs[anno] = {'anno_dfs': anno_dfs_, 'segment_df': segment_df, 'versions_df': versions_df}
...
def load_transcribed_files(transcriptions_dir, return_data=False):
    ...
    asr_dirs = [os.path.join(transcriptions_dir, asr_dir) for asr_dir in asr_dirs]
